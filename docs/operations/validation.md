...
| Excel 解析错误边界 | 损坏、加密及扩展名与内容不符的 XLSX/XLS 在扫描和配置预览均返回安全的 400 |
| 工作簿资源保护 | XLSX ZIP 条目、展开总量、单条目和压缩比受限；XLSX/XLS 的 Sheet、行、列、单元格规模受限；扫描单遍计数且只保留候选/样例 |
...
- 损坏、加密、扩展名伪装和高展开量工作簿的安全拒绝；
- 工作簿 Sheet/行/列/单元格维度上限和单遍流式扫描；
- 多 Sheet 合并预览、10,000 行总上限和来源坐标；
- 最终导入的 Sheet 配置/来源归属与 10,000/10,001/excluded 边界；
...
- 测试仍报告 Starlette `anyio.abc.BlockingPortal` 弃用警告；不影响本次 86 项 Python 测试与 23 项 Node 测试通过。
