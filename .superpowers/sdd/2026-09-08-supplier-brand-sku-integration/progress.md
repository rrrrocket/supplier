...
Final fix re-review: 0 Critical, 2 Important and 2 Minor open — wall-clock-only watermarks could miss a transaction flushed before and committed after the first page; server/browser integer parsing diverged; internal list cursors did not bind organization; future `updated_since` moved the checkpoint backward.

Ruling: Establish public incremental-sync watermarks with PostgreSQL transaction advisory fencing: every Organization, SupplierProfile, Brand, SupplierBrandCooperation, SupplierSku, and Product write takes the shared fence and receives database `updated_at`, while a first list page takes the exclusive fence before reading and publishing its watermark. Cost if wrong: the first page can wait for an active catalog writer, and these six trigger installations plus the shared numeric lock ID must remain aligned with the application.

Ruling: Import integer fields use one exact cross-language grammar: optional thousands separators are normalized; whole integers, `.0` forms, and an optional letters-only unit suffix are accepted within PostgreSQL Integer range `0..2147483647`; fractional values, scientific notation, mixed numeric suffixes, negative values, and overflow are rejected. Python and browser tests consume one shared vector file. Cost if wrong: previously tolerated silent truncation such as `1.5 -> 1` becomes a correction-row error.

Ruling: Internal Product/Offer page cursors bind the authenticated supplier organization in addition to resource and filters. Cost if wrong: cursors cannot survive a supplier-session change and must restart from page one, which prevents cross-session page splicing.

Ruling: Reject `updated_since` values later than the commit-safe server watermark with HTTP 400 `FUTURE_SYNC_WATERMARK` instead of moving a caller checkpoint backward or inventing a future fence. Cost if wrong: callers with a fast local clock must correct it or resume from their last server-issued watermark.

Final fix re-review corrections: implementation complete, pending final repeated verification (commit-safe writer fence revision `d14f0c6a7e92`; shared integer vectors; tenant-bound internal cursors; future-watermark rejection).

Post-review verification: two consecutive `PYTHONDONTWRITEBYTECODE=1 ./start.sh test` runs passed on the same retained database (271 Python tests; 57 Node tests, 0 failed). The barrier regression proves a first list page waits for a pre-existing uncommitted Supplier writer, includes it once after commit, and excludes it from the next round. Shared import vectors pass in Python and Node. Alembic reports sole head/current `d14f0c6a7e92`, the sync-fence revision upgrades and downgrades independently, and `alembic check` reports no drift. Compose configs, compileall, shell/JavaScript syntax, seven guide JSON examples, and diff checks pass. The retained test database is healthy, six checked business tables are empty, and no runner remains.
